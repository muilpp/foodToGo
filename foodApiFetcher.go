...
func getFood(bearerToken string) []string {
	apiUser := os.Getenv("API_USER")
	apiPassword := os.Getenv("API_PASSWORD")

	if bearerToken == "" {
		newBearer := getAuthBearer(apiUser, apiPassword)
		writeBearerToFile(newBearer)
	...
	req, err := http.NewRequest("POST", "https://apptoogoodtogo.com/api/item/v7/discover", bytes.NewBuffer(jsonStr))
	if err != nil {
		panic(err)
	}
	...
	if resp.StatusCode == 401 && currentTries < MAX_TRIES {
		...
		authBearer := getAuthBearer(apiUser, apiPassword)
		writeBearerToFile(authBearer)
		getFood(authBearer)
	...
	for _, grouping := range responseStruct.Groupings {
